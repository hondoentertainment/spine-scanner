...
1. **Confidence scoring** ✅ Done — `ScanConfidenceBand` (`low` / `medium` / `high`) computed in `useScanPipeline` and surfaced in the Scanner UI via the `scanHealthCard` panel. Tests exist in `useScanPipeline.test.ts`.
2. **Regression fixture test suite** ✅ Done — `src/hooks/__tests__/scanRegressionFixtures.test.ts` covers: dim-light capture, glossy/over-exposed cover, partial barcode with checksum-repair, rotated spine (90°/270° pass), low-resolution (move-closer), and confidence band mapping (low/medium/high).
3. **OCR diagnostics panel** ✅ Done — `DebugPanel.tsx` exposes live telemetry and timestamped scan logs, toggled by the terminal icon in the scanner toolbar.
4. **Device benchmark runner** ✅ Done — `scripts/benchmark-scan.ts` (`npm run benchmark:scan`) runs the scan pipeline's deterministic planning layer against the difficult-capture fixture set and writes `benchmark-results/scan-benchmark.csv`. OCR pass count per fixture is the time-to-add proxy and is diffable across pipeline changes.
